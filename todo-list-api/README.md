...
- Update task `/todos/<id>` [PUT]: Protected. Update a task owned by the user.
	...
	```json
	{
		...
		"status": "todo",
		"created_at": "2026-04-26T12:34:56+00:00",
		"updated_at": "2026-04-26T12:40:56+00:00"
	}
	```
...
- List tasks `/todos` [GET]: Protected. Supports pagination, filtering and sorting.
	...
	- `dates`: inclusive date range in `YYYY-MM-DD` format separated by comma. If only a date provided, it filters that single day.
	- `sortBy`: sort by `createdAt` or `updatedAt`. 
	- `sortOrder`: `asc` or `desc` (default `asc`)

	Example request:

	```bash
	curl "http://localhost:5000/todos?page=1&limit=10&search=milk&sortBy=createdAt&sortOrder=desc" \
		-H "Authorization: Bearer <ACCESS>"
	```
